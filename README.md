...
后来我想出了一个方案：[尝试加载一千张照](https://www.jianshu.com/p/f77b9f68265d)。 其实主要就是用自己在艺术探索中学习的知识，结合像线程池、LruCache等一定程度上把照片流畅的加载出来了。
...
大量图片的加载很容易出现ANR和OOM，要控制Bitmap的采样，以及线程和合理切换。整个架构要考虑代码的扩展性。
